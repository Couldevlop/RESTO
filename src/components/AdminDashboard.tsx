...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        console.log(data);
        setOrders(data);
        calculateInitialTotal(data); // Calculer le total initial
      } catch (error) {
        console.error(error);
      }
    };
  ...
  // Calculer le total des commandes servies
  ...
  const handleUpdateStatus = async (
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/resto/api/orders/${orderId}`,
        {
          method: "PUT",
      ...
      setOrders((prevOrders) =>
        prevOrders.map((order) => {
          if (order.id === orderId) {
            ...
            } else if (order.status === "Servi") {
              const orderTotal = order.items.reduce(
                (sum, item) => sum + item.price * item.quantity,
                0
              );
              setTotalServed((prevTotal) => prevTotal - orderTotal); // Soustraire le total
            }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="text-2xl text-amber-800">
                Total Servi: {totalServed.toLocaleString()} CFA
              </p>
            </div>
