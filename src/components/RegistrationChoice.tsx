...
const RegistrationChoice: React.FC = () => {
  ...
  // Fonction pour gérer le choix et rediriger
  const handleRegistrationType = (type: "client" | "personnel") => {
    if (type === "client") {
      navigate("/registration"); // Redirection vers les catégories
    } else {
      navigate("/admin-dashboard"); // Redirection vers AdminDashboard
    }
  };
  ...
  return (
    <div className="min-h-screen bg-amber-50 flex items-center justify-center p-6">
      <div className="w-full max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
          <button
            onClick={() => handleRegistrationType("client")}
            className="p-8 bg-amber-800 text-white rounded-lg shadow-lg hover:bg-amber-900 transition-all"
          >
            <h2 className="text-2xl font-bold mb-2">CLIENTS</h2>
            <p className="text-sm opacity-80">
          ...
          <button
            onClick={() => handleRegistrationType("personnel")}
            className="p-8 bg-blue-600 text-white rounded-lg shadow-lg hover:bg-blue-700 transition-all"
          >
            <h2 className="text-2xl font-bold mb-2">PERSONNEL</h2>
            <p className="text-sm opacity-80">
